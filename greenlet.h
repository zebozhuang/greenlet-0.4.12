...
typedef struct _greenlet {
	PyObject_HEAD
	char* stack_start;			/* 栈开始 */
	char* stack_stop;			/* 栈停止 */
	char* stack_copy;			/* 栈拷贝 */
	intptr_t stack_saved;		/* 栈是否保存 s*/
	struct _greenlet* stack_prev;	/* 前一个greenlet地址 */
	struct _greenlet* parent;		/* 父亲parent地址 */
	PyObject* run_info;				/* 当前运行的信息 */
	struct _frame* top_frame;		/* Python帧栈 */
	int recursion_depth;			/* 递归深度 */
	PyObject* weakreflist;			/* 引用列表 */
	PyObject* exc_type;				/* 类型 */
	PyObject* exc_value;			/* 值 */
	PyObject* exc_traceback;		/* 异常追踪 */
	PyObject* dict;					/* ? */
} PyGreenlet;
