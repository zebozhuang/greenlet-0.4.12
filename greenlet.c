...
Greenlet的数据被保存在栈或者堆里面，最近的会被保存在堆里面(为什么?), 旧的数据会被保存在栈里面
...
Greenlet是被链接起来的，当前运行greenlet就是第一个元素,初始化的greenlet是最后一个
...
States:
  stack_stop == NULL && stack_start == NULL:  did not start yet  没有开始状态
  stack_stop != NULL && stack_start == NULL:  already finished   已经完成
  stack_stop != NULL && stack_start != NULL:  active             活动状态
...
/* volatile是一个类型修饰符（type specifier），就像大家更熟悉的const一样，
...
green_updatecurrent_restart:
	...
	tstate = PyThreadState_GET();  /* 返回线程当前的状态PyThreadState */ 
